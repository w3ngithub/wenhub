...
import { MenuFoldOutlined, MenuOutlined, CloseOutlined, DownOutlined } from '@ant-design/icons'
...
function NavBar({ navItems, backgroundColor, styles }) {
  ...
  return (
    ...
    {
      <div className={`${showMenuMobile ? style.mobileMenuContainerShow : style.mobileMenuContainerHide} ${style.mobileView}`}>
        <CloseOutlined className={style.closeBtnMob} onClick={() => setShowMenuMobile(false)} />
        <ul style={{paddingTop: '25px'}}>
        {
          navItems.map((item) => 
          !item.subItem ?
            <li className={style.listMobile} onClick={() => setShowMenuMobile(false)}>
              <Link href={`${item.path}`}>
                {item.item}
              </Link>
            </li> 
           ...
           <li className={style.listMobile} >
              <span onClick={() => setShowSubItem(!showSubItem)}>{item.item} <DownOutlined style={{fontSize: '11px', marginLeft: '5px'}} /></span>   
              {showSubItem && (
                <ul>
                  {item.subItem.map(sub => 
                    <li className={style.subListMobile} onClick={() => setShowMenuMobile(false)}>
                      <Link href={sub.path}>
                        {sub.item}
                      </Link>
      ...
      >
        <div className={style.headerContainer}>
          ...
          <div className={`${style.menuOpen} ${style.mobileView}`}>
            <MenuOutlined style={{ fontSize: '16px', color: '#fff'}} onClick={() => setShowMenuMobile(true)} />
          </div>
        </div>
              ...
              item.subItem ? (
                ...
                >
                  {item.subItem.map((subitem) => (
                    <Menu.Item key={subitem.id} className={style.sub_menu_container} style={{ backgroundColor, width: '100%', textAlign: 'left' }}>
                      <Link href={subitem.path}>
                        <span  className={style.sub_menu_item}>{subitem.item}</span>
                      </Link>
                    </Menu.Item>
                  ))}
                </SubMenu>
